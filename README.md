...
The project is organized as a multi-module Maven application so persistence, domain logic, REST endpoints, the runnable application, and order-processing concerns can evolve independently.
...
SimpleDeviceManagement/
  SimpleDeviceManagementApp/   Spring Boot application entrypoint, runtime config, and BDD tests
  SimpleDeviceManagementCore/  REST controllers, services, mappers, models, repositories, and security
  SimpleDeviceManagementDb/    PostgreSQL and Oracle schema/data scripts
...
The API is protected by HTTP Basic authentication. The current development credentials are defined in `SimpleDeviceManagementCore/src/main/java/.../SecurityConfig.java`.
...
BDD tests are located in `SimpleDeviceManagementApp/src/test/resources/features`.
